      ...
      def GetSigmaD(self, param): 
          ...
          for  dt in self.dates:
               ...
               if len(fg) !=0 and len(an)!= 0: 
                  ...
                  if  param  =="t" : 
                     diag_so_dict[dt] = self.ComputeSigmao( fg, an )
                     diag_sb_dict[dt] = self.ComputeSigmab( fg, an )
                  elif param =="bt":
                     diag_so_dict[dt] = self.ComputeSigmao( fg, an )
                     diag_sb_dict[dt] = self.ComputeSigmab( fg, an )
...
class RatiosByDate:
    def __init__(self, paths, dates, so_pred):
#        Ratios.__init__(self )
        self.so_pred = so_pred 
        print( self.so_pred)  
        return None 

#    def GlobalRatio (self ):
#        print( r.rednmc  )
        

#class RatiosByDate(Ratios):
#    r=Ratios (paths  = {"BASEDIR":None} ,
#                      Nobs   =None ,
#                      rednmc =None ,
#                      so_pred=None ,
#                      so_diag=None ,
...
#    def __init___(self ):
        #super().__init__(rednmc)
        #self.rednmc   =rednmc 
        #print("-----", self.rednmc )
#        self.so_pred  =so_pred
#        self.so_diag  =so_diag 

#        self.sb_pred  =sb_pred
#        self.sb_diag  =sb_diag
#        return None 
